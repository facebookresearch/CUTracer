...
class StreamingGrouper:
    ...
    def __init__(self, records: Iterator[dict], group_field: str):
        """
    ...
    def head_per_group(self, n: int) -> dict[Any, list[dict]]:
        ...
        for record in self._records:
            ...
            # Only add if we haven't reached N for this group
    ...
    def tail_per_group(self, n: int) -> dict[Any, list[dict]]:
        ...
        # Use deque with maxlen to automatically discard old records
    ...
    def collect_all(self) -> dict[Any, list[dict]]:
        """
        Collect all records per group.

        WARNING: This loads all records into memory!
        Use head_per_group() or tail_per_group() for large files.

        Memory complexity: O(total records)

        Returns:
            Dict mapping group key to list of all records
        """
        self._ensure_not_consumed()
