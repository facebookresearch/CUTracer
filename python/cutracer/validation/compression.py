...
def detect_compression(filepath: Union[str, Path]) -> str:
    ...
    Uses both file extension and magic number detection for reliability.
    ...
    if suffixes.endswith((".zst", ".zstd", ".ndjson.zst", ".ndjson.zstd")):
    ...
    # Check magic number for files without extension hint
    with open(filepath, "rb") as f:
        magic = f.read(4)
        if magic == ZSTD_MAGIC:
            return "zstd"
...
def get_file_size(filepath: Union[str, Path], compressed: bool = True) -> int:
    ...
    # Count bytes by reading through the file
    total_size = 0
    with open_trace_file(filepath) as f:
        for line in f:
