...
def inspect_stages_hook(
    ...
    stage_src = (
        "from triton._C.libtriton import ir, passes, llvm, amd, nvidia, tlx\n"
        + stage_src
    )
    ...
    stage_src = (
        "from " + make_llir_owner_mod.__name__ + " import CUDABackend\n" + stage_src
    )
    ...
    if not hasattr(make_llir_owner_mod, "get_ptx_version_from_options"):
        raise RuntimeError(
            "get_ptx_version_from_options not found in module defining make_llir"
        )
    get_ptx_version_from_options = getattr(
        make_llir_owner_mod, "get_ptx_version_from_options"
    )
    ...
    stage_src = (
        "from "
        + make_llir_owner_mod.__name__
        + " import get_ptx_version_from_options\n"
        + stage_src
    )
    ...
    if not hasattr(make_llir_owner_mod, "sm_arch_from_capability"):
        raise RuntimeError(
            "sm_arch_from_capability not found in module defining make_llir"
        )
    ...
    stage_src = (
        "from "
        + make_llir_owner_mod.__name__
        + " import sm_arch_from_capability, get_features\n"
        + stage_src
    )
    ...
    make_lambda = lambda f: lambda src, metadata: f(
        self=self, src=src, metadata=metadata, options=options, capability=capability
    )
