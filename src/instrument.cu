...
void instrument_opcode_only(Instr* instr, int opcode_id, CTXstate* ctx_state) {
  // TODO: Implement lightweight opcode-only instrumentation
  // This should be much more efficient than the current dual instrumentation
  // 
  // Proposed implementation:
  // 1. Insert call to a new "collect_opcode_only" device function
  // 2. Only pass essential parameters:
  //    - predicate value
  //    - opcode_id  
  //    - kernel_launch_id (for boundary detection)
  //    - channel pointer
  // 3. Skip register values and memory addresses entirely
  //
  // Example skeleton:
  // nvbit_insert_call(instr, "collect_opcode_only", IPOINT_BEFORE);
  // nvbit_add_call_arg_guard_pred_val(instr);
  // nvbit_add_call_arg_const_val32(instr, opcode_id);
  // nvbit_add_call_arg_launch_val64(instr, 0);  // kernel_launch_id
  // nvbit_add_call_arg_const_val64(instr, (uint64_t)ctx_state->channel_dev);
  
  // For now, leave empty for you to implement
}

void instrument_register_trace(Instr* instr, int opcode_id, CTXstate* ctx_state,
                              const std::vector<int>& reg_num_list,
